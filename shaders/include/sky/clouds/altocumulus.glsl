...
float clouds_altocumulus_density(vec3 pos) {
	...
	vec3 curl = (0.181 * CLOUDS_ALTOCUMULUS_CURL_STRENGTH) * texture(colortex7, 0.015 * pos).xyz * smoothstep(0.4, 1.0, 1.0 - altitude_fraction);
	...
	density -= (0.8 * CLOUDS_ALTOCUMULUS_DETAIL_STRENGTH) * sqr(worley) 
...
#endif // INCLUDE_SKY_CLOUDS_ALTOCUMULUS